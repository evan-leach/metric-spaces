import React, { useMemo } from 'react'
import BaseScene from '../../components/BaseScene'
import Blob from '../../components/objects/Blob'
import Point from '../../components/objects/Point'
import Circle from '../../components/objects/Circle'
import PointSequence from '../../components/objects/PointSequence'
import { useKeyframeAnimation } from '../../hooks/useKeyframeAnimation'
import { colors } from '../../config/colors'

export const SceneConfig = {
  zoom: { min: 0.2, max: 50000 },
  steps: [
    "TOP: At the start of this section, we saw elements of a set $A$ that _looked_ different and turned\
    our qualitative observations into precise definitions for interior, exterior, and boundary points.",
    "TOP: For example, these two points _look_ different, and we can indeed distinguish them. One is an interior point, and one\
    is a boundary point.",
    "TOP: However, we don't yet have a way to distinguish these two points from each other, even though they also look different.\
    They're both boundary points of $A$, since any neighboorhood of either point contains at least one element of $A$ and of $A^c$.\
    $$$$ Can you think of a precise condition that can tell these two points apart?",
    "TOP: Here's an observation: one point only lies on the boundary because the _point itself_ is contained in all of its\
    neighborhoods. The other point would still lie on the boundary even if it wasn't an _element_ of $A$.",
    "This leads us to the following definition: $$$$ A point $x$ is a *limit point* of a set $A$ if every neighborhood of $x$\
    contains a point of $A$ _other than_ $x$ _itself._",
    "TOP: This is how we disambiguate between the two boundary points. Any neighborhood of $x$ contains elements of $A$\
    other than $x$...",
    "TOP: ...and while some neighborhoods of $y$ contain elements of $A$ other than $y$...",
    "TOP: ...a small enough neighborhood will contain _only_ $y$. $$$$ This means that $x$ is a limit point of $A$, and $y$ is not.",
    "TOP: Not all limit points are necessarily elements of $A$, as this example shows.",
    "TOP: The two limit points we've seen so far had _other limit points_ as neighbors. This point, however, is different.\
    $$$$ Although the points of $A$ get closer and closer to it, none of the other points in this cluster are _limit points._\
    $$$$ Zoom in to convince yourself of this, and zoom back out when you're done.",
    "Suppose $x$ is a limit point of a set $A$. The definition of limit points tells us that every neighborhood of $x$ contains at least one\
    point of $A$ other than $x$ itself. However, something much more suprising is also true: $$$$ Every neighborhood of $x$ must\
    contain _infinitely many_ points of $A$. $$$$ How can this be? And why is infinity showing up?",
    "TOP: Even if a neighborhood of some point $y$ contains a point in $A$ other than $y$, it might not contain more than one, let alone\
    _infinitely many._ $$$$ Here is an example. Does this disprove our claim that every neighborhood of a limit point contains\
    infinitely many points of $A$?",
    "TOP: Not quite. If we make the neighborhood smaller, it no longer contains _any_ points of $A$ other than $y$. This means that\
    it's not actually a limit point, since it's not true that _every_ neighborhood of $y$ contains a point of $A$ other than $y$ itself.",
    "The reason $y$ was not a limit point is because we could shrink the neighborhood until it no longer contained any points of $A$\
    other than $y$ itself. $$$$ We can use this idea to prove that if _any_ point $z$ has a neighborhood with only finitely many points\
    of $A$, then $z$ cannot possibly be a limit point. $$$$ Take a moment to convince yourself\
    that this is _equivalent_ to the original claim: that any neighborhood of a limit point contains infinitely many points of $A$.",
    "TOP: Let's pick such a point $z$. Some neighborhoods of $z$ (like this one) might contain infinitely many points points\
    of $A$...",
    "TOP: ...but suppose that some sufficiently small neighborhood contains only finitely many points of $A$.",
    "TOP: Since there are only finitely many points of $A$ in this neighborhood, we can choose the point that is _closest_ to $z$\
    (other than $z$ itself). $$$$ We'll call this point $w$.",
    "TOP: We know that $d(z,w) > 0$, since $z$ and $w$ are distinct points. We can therefore set $r$ to be equal to $d(z,w)$, and $N_r(z)$\
    will still be a valid neighborhood of $z$.\
    $$$$ This neighborhood is the set of all points _closer_ to $z$ than the _closest_ point in $A$, so it doesn't contain any points\
    of $A$ other than $z$ itself. This means that $z$ cannot be a limit point of $A$!",
    "TOP: In other words, the only way a point $x$ can be a limit point is if there _is no_ closest point. Zoom in towards $x$ to convince\
    yourself that no single point of $A$ is closest. $$$$ If any neighborhood of $x$ contained finitely many points of $A$, there _would_ be a\
    closest point. This means that if $x$ really is a limit point, every neighborhood of $x$ must contain infinitely many points of $A$.",
    "This proof gives us an example of how to work with infinity in practice. Rather than trying to imagine infinitely many points, we just\
    consider infinity to be _not finite._ Infinity is how we break the rules that finite sets have to follow (such as having minimal and maximal\
    elements). $$$$ Infinity will become more and more important in the next few sections. However, we will first conclude _this_ section by\
    exploring how limit points relate to interior, exterior, and boundary points. We'll finish with a third equivalent definition of closed sets\
    using limit points."
  ]
}

function LimitPts({ 
  windowSize, 
  stepIndex, 
  isPanelOpen,
  sceneKey 
}) {

  const spiralCurve = (t) => {
    const angle = t + 3.5
    const radius = t * 2.5
    return [
      3.6 - radius * Math.cos(angle),
      radius * Math.sin(angle) - 1.3
    ]
  }

  const chaoticCurve = (t) => {
    const sx = 100 * t
    const sy = 94 * t
    const x = 4 * Math.abs(sx - Math.floor(sx) - 0.5) - 1
    const y = 4 * Math.abs(sy - Math.floor(sy) - 0.5) - 1
    return [
      5 * x * x * x,
      5 * y * y * y
    ]
  }

  const toZeroCurve = (t) => {
    const angle = t * 10000000
    const radius = t * 2.5
    return [
      radius * Math.cos(angle),
      radius * Math.sin(angle)
    ]
  }

  const solidBlobs = useMemo(() => [
    { center: [-6, -5], size: 4, seed: 18, scale: 0.2 },
    { center: [9, 6], size: 7, seed: 478, scale: 0.3 },
    { center: [3, -5], size: 2, seed: 105, scale: 0.15 },
    { center: [0, 0], size: 1.5, seed: 456, scale: 0.25 },
    { center: [-5, 2], size: 1, seed: 457, scale: 0.25 }
  ], [])

  const dashedBlobs = useMemo(() => [
    
    { center: [-3, 1], size: 0.2, seed: 789, scale: 0.2 }
  ], [])

  const points = useMemo(() => [
    [4, -2],
    [4.2, -1.3],
    [1.25, -1],
    [4.3, -2.3],
    [2, 0],
    [4.4, -2.1],
    [-2, 0],
    [-1.5, 1],
    [-2.8, -0.5]
  ], [])

  const colorKeyframes = useMemo(() => ({
    color: {
      2: colors.blue,
      3: colors.red,
      9: colors.blue
    }
  }), [])

  const point3Keyframes = useMemo(() => ({
    color: {
      12: colors.orange,
      14: colors.blue
    }
  }), [])

  const circle1Keyframes = useMemo(() => ({
    radius: {
      5: 0,
      6: 0.6,
      7: 0
    }
  }), [])

  const circle2Keyframes = useMemo(() => ({
    radius: {
      6: 0,
      7: 0.6,
      8: 0.15,
      9: 0
    }
  }), [])
  
  const circle3Keyframes = useMemo(() => ({
    radius: {
      11: 0,
      12: 0.6,
      13: 0.2,
      14: 0
    }
  }), [])

  const circle4Keyframes = useMemo(() => ({
    radius: {
      14: 0,
      15: 1.5,
      16: 0.7,
      18: Math.sqrt(0.01 + 0.04 * 0.04),
      19: 0
    }
  }), [])

  // Use keyframe animation for color
  const animatedProps = useKeyframeAnimation(colorKeyframes, stepIndex)
  const pointColor = animatedProps.color || colors.blue
  const animatedProps3 = useKeyframeAnimation(point3Keyframes, stepIndex)
  const point3Color = animatedProps3.color || colors.blue

  return (

    <BaseScene
      config={SceneConfig}
      sceneKey={sceneKey}
      stepIndex={stepIndex}
      isPanelOpen={isPanelOpen}
      windowSize={windowSize}
    >
      {solidBlobs.map((blob, index) => (
        <Blob
          key={`solid-blob-${index}`}
          center={blob.center}
          size={blob.size}
          harmonicsConfig={{ scale: blob.scale, seed: blob.seed }}
          color={colors.blue}
          style="solid"
          startFrame={0}
          endFrame={10}
          stepIndex={stepIndex}
        />
      ))}

      {dashedBlobs.map((blob, index) => (
        <Blob
          key={`dashed-blob-${index}`}
          center={blob.center}
          size={blob.size}
          harmonicsConfig={{ scale: blob.scale, seed: blob.seed }}
          color={colors.blue}
          style="dashed"
          startFrame={0}
          endFrame={10}
          stepIndex={stepIndex}
        />
      ))}

      {points.map((point, index) => (
        <Point
          key={`point-${index}`}
          center={point}
          stepIndex={stepIndex}
          color={colors.blue}
          startFrame={0}
          endFrame={10}
        />
      ))}
      <Point
        center={[-3, -2.5]}
        stepIndex={stepIndex}
        color={colors.red}
        startFrame={2}
        endFrame={2}
      />
      <Circle
        center={[-1, -1.1262036]}
        keyframes={circle1Keyframes}
        stepIndex={stepIndex}
        color={colors.red}
        strokeStyle='dashed'
      />
      <Circle
        center={[1.3,-0.7]}
        keyframes={circle2Keyframes}
        stepIndex={stepIndex}
        color={colors.orange}
        strokeStyle='dashed'
      />
      <Point
        center={[-1, -1.1262036]}
        stepIndex={stepIndex}
        color={colors.red}
        startFrame={2}
        endFrame={8}
      />
      <Point
        center={[1.3,-0.7]}
        stepIndex={stepIndex}
        color={pointColor}
        startFrame={0}
        endFrame={10}
      />
      <Point
        center={[-1, -1.1262036]}
        stepIndex={stepIndex}
        color={colors.red}
        startFrame={6}
        endFrame={8}
        labelContext={() => `$x$`}
        labelAttach="s"
      />
      <Point
        center={[1.3,-0.7]}
        stepIndex={stepIndex}
        color={colors.orange}
        startFrame={6}
        endFrame={8}
        labelContext={() => `$y$`}
        labelAttach="se"
      />
      <PointSequence
        curve={spiralCurve}
        cutoff={40}
        k={0.6}
        pointColor={colors.blue}
        startFrame={0}
        endFrame={10}
        stepIndex={stepIndex}
      />
      <Point
        center={[-3,0.8101505]}
        stepIndex={stepIndex}
        color={colors.red}
        startFrame={9}
        endFrame={9}
      />
      <Point
        center={[3.6,-1.3]}
        stepIndex={stepIndex}
        color={colors.red}
        startFrame={10}
        endFrame={10}
      />
      <PointSequence
        curve={chaoticCurve}
        cutoff={100}
        k={0.99}
        pointColor={colors.blue}
        startFrame={12}
        endFrame={19}
        stepIndex={stepIndex}
        labelAttach="ne"
        labelPointIndex={5}
        labelContext={() => '$A$'}
      />
      <PointSequence
        curve={toZeroCurve}
        cutoff={100}
        k={0.8}
        pointColor={colors.blue}
        startFrame={12}
        endFrame={19}
        stepIndex={stepIndex}
      />
      <Point
        center={[-1.1,0.14]}
        stepIndex={stepIndex}
        color={colors.blue}
        startFrame={12}
        endFrame={19}
      />
      <Point
        center={[-1.1,0.14]}
        stepIndex={stepIndex}
        color={colors.purple}
        startFrame={17}
        endFrame={18}
        labelContext={() => '$w$'}
        labelAttach="nw"
      />
      <Point
        center={[-2,-1.5]}
        stepIndex={stepIndex}
        color={point3Color}
        startFrame={12}
        endFrame={19}
      />
      <Point
        center={[-1,0.1]}
        stepIndex={stepIndex}
        color={colors.blue}
        startFrame={12}
        endFrame={19}
      />
      <Point
        center={[-1,0.1]}
        stepIndex={stepIndex}
        color={colors.orange}
        startFrame={15}
        endFrame={18}
        labelContext={() => '$z$'}
        labelAttach="se"
        labelAttachDistance={25}
      />
      <Circle
        center={[-1,0.1]}
        stepIndex={stepIndex}
        color={colors.orange}
        keyframes={circle4Keyframes}
        strokeStyle='dashed'
        labelContext={() => '$N_r(z)$'}
        labelAttach="ne"
        labelAttachDistance={35}
        showCenterPoint={false}
      />
      <Circle
        center={[-2,-1.5]}
        stepIndex={stepIndex}
        color={colors.orange}
        keyframes={circle3Keyframes}
        strokeStyle='dashed'
        labelContext={() => '$y$'}
        labelAttach="s"
        labelAttachDistance={25}
        showCenterPoint={false}
        labelAtCenter={true}
      />
      <Point
        center={[0,0]}
        stepIndex={stepIndex}
        startFrame={19}
        endFrame={19}
        color={colors.red}
        labelContext={() => '$x$'}
        labelAttach="n"
      />
      
      
    </BaseScene>
  )
}

export default LimitPts 